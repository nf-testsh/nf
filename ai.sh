...
# --- 核心请求函数 ---
curl_get() {
    curl -4 --user-agent "${UA_Browser}" -s --max-time 10 "$1"
}

# --- ChatGPT ---
Check_ChatGPT() {
    local ios_result=$(curl_get "https://ios.chat.openai.com/public-api/mobile/server_status/v1")
    local trace_result=$(curl -4 --user-agent "${UA_Browser}" -s --max-time 5 "https://chatgpt.com/cdn-cgi/trace")
...
# --- Sora ---
Check_Sora() {
    local trace_result=$(curl -4 --user-agent "${UA_Browser}" -s --max-time 5 "https://sora.com/cdn-cgi/trace")
    local region=$(echo "$trace_result" | grep "loc=" | cut -d= -f2)
    ...
    elif [ "$login_check" == "200" ] || [ "$login_check" == "302" ]; then
...
# --- Gemini ---
Check_Gemini() {
    local tmpresult=$(curl -4 --user-agent "${UA_Browser}" -sL --max-time 10 "https://gemini.google.com")
